...
#     """
#     Draws an 8x10 grid:
#       - The top 8 rows form a chessboard with alternating colors.
#       - The bottom 2 rows are extra rows (light gray background).
#     A red dot is drawn on top of the grid, moving in half‑steps.
#     The grid scales relative to the parent widget and is offset from the bottom-left.
#     When trail mode is active, a continuous red line is drawn connecting each new dot center.
#     """
...
#         # Dot's position in "steps": horizontal (0 to 16) and vertical (0 to 20)
#         self.dot_step_x = 8
#         self.dot_step_y = 11  
#         # Offset (in pixels) from the widget's bottom-left
#         self.offset_value = 50  
#         # Trail points (each is a tuple of (x, y) in pixels)
#         self.trail_points = []
#         # If True, new dot positions are added to the trail.
...
#         self.canvas.clear()
#         # Compute square size relative to parent's size minus the offset.
...
#         # Board origin: offset from the widget's bottom-left.
#         board_origin = (self.x + self.offset_value, self.y + self.offset_value)
        
#         with self.canvas:
#             # --- Draw the extra two rows (bottom rows 0 and 1) ---
#             for row in range(2):
#                 for col in range(self.cols):
#                     x = board_origin[0] + col * self.square_size
#                     y = board_origin[1] + row * self.square_size
...
#                 x = board_origin[0] + col * self.square_size
#                 Line(points=[x, board_origin[1], x, board_origin[1] + self.rows * self.square_size], width=1)
#             for row in range(self.rows + 1):
#                 y = board_origin[1] + row * self.square_size
#                 Line(points=[board_origin[0], y, board_origin[0] + self.cols * self.square_size, y], width=1)
...
#             # --- Draw the red dot ---
#             dot_center_x = board_origin[0] + self.dot_step_x * self.step_size
#             dot_center_y = board_origin[1] + self.dot_step_y * self.step_size
...
#         """
#         Moves the red dot by one half‑step in x and/or y (25mm per step).
#         Clamps the dot's position within the grid.
#         If trail mode is active, the new center is added to the trail.
#         """
...
#         board_origin = (self.x + self.offset_value, self.y + self.offset_value)
#         new_center = (board_origin[0] + self.dot_step_x * self.step_size,
#                       board_origin[1] + self.dot_step_y * self.step_size)
...
class GantryControlWidget(BoxLayout):
    ...
    def __init__(self, **kwargs):
        ...
        c = BoxLayout(orientation='vertical', size_hint=(0.1, 0.1))
        ...
        c.add_widget(self.pathButton)
        c.add_widget(c_clear)
...
# Instantiate the modular chess move input.
    ...
    def on_move_entered(self, move):
        ...
        if self.target_board.trail_points:
            ...
            self.chess_move_input.text = "Path: " + " -> ".join(coords)
...
class ChessMoveInput(TextInput):
    ...
    def on_text_change(self, instance, value):
        if self.path_button:
            # Disable the path button if there's any non-whitespace text.
