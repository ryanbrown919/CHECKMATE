...
class GameLogic:
    ...
    def start_game(self, opponent="bot", **kwargs):
        ...
        self.start_listening_for_moves()
    ...
    def push_move(self, move_str):
        ...
        # If move_input is already a chess.Move, use it directly.
        if isinstance(move_str, chess.Move):
            move = move_str
        else:
            # Otherwise, assume it's a string and try parsing it.
            try:
                ...
                # Fallback: parse as SAN.
                if move is None:
                    move = self.game_board.parse_san(move_str)

                if move not in self.game_board.legal_moves:
                    raise ValueError("Illegal move: " + str(move_str))
    ...
    def handle_opponent_move(self, move_str):
        ...
        self.game_board.push(move)
        self.game_history.append(move)
        print(f"Received opponent move: {move_str}")
        self.notify_observers()
        self.check_game_status()
    ...
    def start_listening_for_moves(self):
        """
        Start a background thread to listen for incoming moves from Lichess.
        This method processes game events and calls handle_opponent_move for any new moves.
        """
    ...
    def _listen(self):
        """Continuously listen to the Lichess game stream for new moves."""
        for event in self.game.stream_game():
            if event.get("type") == "gameState":
                moves_str = event.get("moves", "")
                moves_list = moves_str.split() if moves_str else []
                # If the stream returns more moves than our history, process the new ones.
                if len(moves_list) > len(self.game_history):
                    for move_str in moves_list[len(self.game_history):]:
                        self.handle_opponent_move(move_str)
